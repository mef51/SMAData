pro uti_xdelay, reference=reference, defaults=defaults, _extra=extra_keywords
...
result=dat_filter(s_f,'("iaq" eq "1" and "ibq" eq "0" and "wt" gt "0")',/no_notify,/reset)
...
if not keyword_set(defaults) then begin
   ...
   result=pass_xdelay(temp_souid,'channel','pha',smoothing,'blcd,rec,sb,band', $
     4,1,funct=funct,npoly=3,_extra=extra_keywords)
   aa = ''
...
result=dat_filter(s_f,'("iaq" eq "0" and "ibq" eq "1" and "wt" gt "0")',/no_notify,/reset)
if result le 0 then begin
