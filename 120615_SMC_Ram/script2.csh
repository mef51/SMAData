...
set regdisp=quart
if($source == "smc.8") set regdisp='arcsec,box(-8,-8,16,16)'
set niters=4000
#set gain=0.3
#set so=1733-130_$dt
...
set so="smc.8"
\rm -fr $so.stokes
uvaver vis=combined.lsb.bp out=$so.lsb.stokes select="so(${so})" interval=5
\rm -fr $so.stokes.chi
uvredo vis=$so.lsb.stokes options=chi out=$so.lsb.stokes.chi
...
if($source == "smc.8") set sigmaQ=0.0018
\rm -fr $so.poli $so.polierr $so.polm $so.polmerr $so.pa $so.paerr $so.unif
impol in=$so.q.mp,$so.u.mp,$so.i.cm sigma=$sigmaQ,$sigmaQ sncut=2.5,$sncutI \
