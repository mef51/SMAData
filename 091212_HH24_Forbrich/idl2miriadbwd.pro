...
;      file name for the antenna position

;      default is the "antennas" file under the raw data directory
...
;c    rapp,dapp  Geocentric apparent (RA,DEC).
        ...
        if (strpos(!VERSION.ARCH,'86') ge 0) then begin
            ...
            if (strpos(!VERSION.ARCH,'64') ge 0) then begin
                ...
                result=CALL_EXTERNAL('libidlmir-bwd.so', $
...
pro idl2miriadbwd,dir=dir,source=source,sideband=sideband,band=band, $

               cont=cont,edge_trim=edge_trim,polar=polar,antpos=antpos, $

               verbose=verbose,oldweight=oldweight
...
endelse
  ...
  PRINT," USAGE: idl>idl2miriad-bwd,dir='outputdir',source='sourcename'

  PRINT,"          (,sideband='u',band='s1',edge_trim=0.1,polar=0,/antpos,/verbose)"

  PRINT,"    or: idl>idl2miriad-bwd,dir='outputdir',source='sourcename'

  PRINT,"          (,sideband='u',/cont,edge_trim=0.1,polar=0,antpos='antfilename',/verbose)"
...
; ***** PREP WORK *****
  ...
  ; ***** check antpos keyword *****



  if (not keyword_set(antpos)) then begin

    antpos = 0

    print,"Antenna position WILL NOT be recoreded in MIRIAD header file"

  endif else begin

    if (antpos eq '1') then antpos = "antennas"

    if (file_test(antpos) ne 1) then begin

      print, "The antenna position file '",antpos,"' does not exist!!!"

      return

    endif else begin

      print, "Will use antenna position file '",antpos,"' for MIRIAD header."

      print, "Please note that no further check will be done over the format of"

      print, "the antenna position file. The program assumes the file follows"

      print, "the standard SMA antenna position file."

      print, "CAUTION : Please use this option with caution, especially if the"

      print, "antenna position information has been adjusted in the MIR."
  ...
  if (polar eq 0) then print,"The MIR data are considered as non-polarization data, will use XX as default."

  if (polar eq 1) then print,"The polarization codes in MIR data are considered as circularly polarization."

  if (polar eq 2) then print,"The polarization codes in MIR data are considered as linearly polarization."
...
; ***** observational setup information *****
  ...
  ; *** antenna position info ***



  antposgeo = make_array(nants,3,/double,value=0.0)

  antposequ = make_array(nants,3,/double,value=0.0)

  refantok=0 & refant=0

 

  openr,unit,antpos,/get_lun,error=err
  ...
  for antcnt = 0,(nants-1) do begin

    readf,unit,na,nx,ny,nz

;    print,na,nx,ny,nz

;    print, format = '(I2,"  ",I2,"  ",F0,"  ",F12)', na,nx,ny,ny
    ...
    if ((refantok eq 0) and ((nx*nx+ny*ny+nz*nz) eq 0)) then begin
  ...
  if (refantok eq 0) then print,"No reference antenna found in antenna position file, use AN1"



    ; *** converting geocentric coordinates (in meter, antposin) to ***

    ; *** equatorial coordinates (in nanosec, antposdata) ***

    ; *** based on code in uvedit.for in MIRIAD  ***



  r= sqrt(antposgeo[(refant-1),0]^2+antposgeo[(refant-1),1]^2)

  if (r gt 0) then begin

    cost=antposgeo[(refant-1),0]/r

    sint=antposgeo[(refant-1),1]/r

    z0  =antposgeo[(refant-1),2]
  ...
  DCMKS=double(299792458)



  for antcnt = 0,(nants-1) do begin

    tmpval = antposgeo[(antcnt),0]*cost + antposgeo[(antcnt),1]*sint - r

    antposequ[(antcnt),0] = (1E9/DCMKS) * tmpval

    tmpval = -antposgeo[(antcnt),0]*sint + antposgeo[(antcnt),1]*cost - r

    antposequ[(antcnt),1] = (1E9/DCMKS) * tmpval

    antposequ[(antcnt),2] = (1E9/DCMKS) * (antposgeo[(antcnt),2]-z0)

  endfor



  antposequ = transpose(antposequ)
...
;                     'idl_uvputvr',unit,'r','jyperk',jyperk,one)
  ...
  print,"---          antenna position header info ---"



    result=CALL_EXTERNAL(libfile, $

                     'idl_uvputvr',unit,'d','antpos',antposequ,nants*3)



  print,"---                 fake corr header info ---"
