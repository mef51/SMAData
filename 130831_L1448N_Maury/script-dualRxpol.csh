...
uvaver vis=3c84_"$suffix".$sb,1751+096_"$suffix".$sb,0102+584_"$suffix".$sb,1927+739_"$suffix".$sb,B335_"$suffix".$sb,CB230_"$suffix".$sb,L1157_"$suffix".$sb,L1448-2A_"$suffix".$sb,titan_"$suffix".$sb out=combined_"$suffix".$sb
end
goto terminate
...
SPLIT:
foreach sb(lsb usb)
foreach so(titan 1751+096 B335 1927+739 L1157 CB230 0102+584 3c84 L1448-2A L1448N)
echo $so
...
set visin=L1448-2A"_"$suffix.$band
set stokesout=$so"_"$suffix.$band.stokes
...
SPEC:
set vis=l1448n_4x4.usb.stokes

uvspec device=1/xs axis=fre,real nxy=1,1 interval=1000 stokes=i,q,u,v vis=$vis line=chan,100,100 #options=avall,nobase
goto terminate


POLCAL:
foreach sb(lsb usb)
gpcal vis=$polcal"_""$dt"_"$suffix".$sb  refant=4 options=circular,qusolve interval=10 flux=6.5 select='time(14:30,16:00)'
end
goto terminate
