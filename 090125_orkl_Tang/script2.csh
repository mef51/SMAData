...
smalod in=/sma/data/science/mir_data.2009/$inname out=$dt rxif=1 options=circular,debug sideband=2  refant=1
...
foreach sb(lsb usb)
\rm -fr $fname.$sb.tsys
smafix vis=$fname.$sb.flgd out=$fname.$sb.tsys options=tsyscorr  dofit=2 options=tsyscorr,dosour nxy=1,1 yrange=50,600 nxy=2,4 device=/xw
end
goto terminate
...
\rm -fr $stokesout
uvaver vis=$visin out=$stokesout interval=5
\rm -fr $chiout
