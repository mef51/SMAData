...
uvaver vis="$so""_""$dt".$sb.pf select='so('$so')' interval=6 out="$so""_""$dt".$sb line=ch,1,1,3072
...
invert vis=$so.$sb imsize=256 cell=0.25 options=systemp sup=0 robust=1.0 \
...
clean map=$so.$sb.$pol.mp beam=$so.$sb.bm  \
      out=$so.$sb.$pol.cl region=$region niters=10000 cutoff=0.005
...
invert vis=$vis  \
       map=$so.i.mp,$so.q.mp,$so.u.mp,$so.v.mp \
       stokes=i,q,u,v options=systemp sup=128 robust=0.0 imsize=256 cell=0.15 \
       beam=$so.bm
